...
describe Recommendations::Job do
  ...
  let(:root_job) { create(:job, uid: 1337, dev_type: 2) }
  ...
  describe "#similar_jobs" do
    ...
    it %{
      ...
      applicant_1 = create(:user, uid: "2")
      applicant_2 = create(:user, uid: "3")

      # this applicant applied to a recommended job, but not the root_job
      random_applicant = create(:user, uid: "4")

      # applicant_1, applicant_2 and random_applicant applied to it
      first_job = create(:job, uid: 1, dev_type: 2)
      # applicant_1 and applicant_2 applied to it
      second_job = create(:job, uid: 2, dev_type: 2)
      # only applicant_1 applied to it
      third_job = create(:job, uid: 3, dev_type: 2)
      # not recommended because the current user already applied to it
      excluded_job = create(:job, uid: 4, dev_type: 2)
      # should be obvious
      lonely_job = create(:job, uid: 5, dev_type: 2)
      # wrong dev_type
      wrong_dev_type_job = create(:job, uid: 6, dev_type: 3)
      # draft job
      draft_job = create(:draft_job, uid: 7, dev_type: 2)
      ...
      draft_job.applicants << applicant_1
